...
void printUsage(){
	...
	printf("lsb.exe update InputFname UpdateFname OutputFname\n");
...
int main(int argc, char** argv){
    ...
    if( (strcmp(argv[1],"encode") == 0) ){
		...
		memset(outFileName, 0, 300);
		strcpy(inFileName, argv[2]);
		strcpy(upFileName, argv[3]);
		strcpy(outFileName, argv[4]);

		printf("\n\nUPDATE Mode Entered.\n\n");

		/* Open the input/update/output files */
		inFile = upFile = outFile = NULL;
		...
		/* Parse the Input File for Updating */
		rval = updateScript(inFile, upFile, outFile);
		if (rval == 0){
